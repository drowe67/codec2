...
int main(int argc, char *argv[]) {
  ...
  VLA_CALLOC(int, tx_bits, ofdm_samplesperframe);
  VLA_CALLOC(COMP, tx, ofdm_samplesperframe); /* one frame of tx samples */

  VLA_CALLOC(int, rx_bits, ofdm_bitsperframe); /* one frame of rx bits    */
  printf("Nc = %d ofdm_bitsperframe: %d\n", ofdm_nc, ofdm_bitsperframe);

  /* log arrays */

  VLA_CALLOC(int, tx_bits_log, ofdm_bitsperframe *NFRAMES);
  VLA_CALLOC(COMP, tx_log, ofdm_samplesperframe * NFRAMES);
  VLA_CALLOC(COMP, rx_log, ofdm_samplesperframe * NFRAMES);
  VLA_CALLOC(COMP, rxbuf_in_log, ofdm_max_samplesperframe * NFRAMES);
  VLA_CALLOC(COMP, rxbuf_log, ofdm_nrxbuf * NFRAMES);
  VLA_CALLOC_DIM2(COMP, rx_sym_log, (ofdm_ns + 3) * NFRAMES, ofdm_nc + 2);
  VLA_CALLOC_DIM2(float, phase_est_pilot_log, ofdm_rowsperframe *NFRAMES,
                 ofdm_nc);
  VLA_CALLOC(COMP, rx_np_log, ofdm_rowsperframe * ofdm_nc * NFRAMES);
  VLA_CALLOC(float, rx_amp_log, ofdm_rowsperframe *ofdm_nc *NFRAMES);
  float foff_hz_log[NFRAMES];
  VLA_CALLOC(int, rx_bits_log, ofdm_bitsperframe *NFRAMES);
  ...
  VLA_CALLOC(float, symbol_likelihood_log,
  ...
  for (f = 0; f < NFRAMES; f++) {
    ...
    if (ldpc_enable) {
      ...
      VLA_CALLOC(uint16_t, r, Npayloadbits);
      VLA_CALLOC(uint8_t, payload_bits, Npayloadbits);
  ...
  int lnew;
  VLA_CALLOC(COMP, rxbuf_in, ofdm_max_samplesperframe);

#define FRONT_LOAD
#ifdef FRONT_LOAD
  for (i = 0; i < nin; i++, prx++) {
    ofdm->rxbuf[ofdm_nrxbuf - nin + i] = rx_log[prx];
  }
#endif
...
// #define TESTING_FILE
  ...
  for (f = 0; f < NFRAMES; f++) {
    ...
    VLA_CALLOC(float, symbol_likelihood,
               (CODED_BITSPERFRAME / ofdm_bps) * (1 << ofdm_bps));
    ...
    if (ldpc_enable) {
      ...
      VLA_CALLOC(COMP, ldpc_codeword_symbols, (CODED_BITSPERFRAME / ofdm_bps));

      for (i = 0, j = (ofdm_nuwbits + ofdm_ntxtbits) / ofdm_bps;
           i < (CODED_BITSPERFRAME / ofdm_bps); i++, j++) {
        ldpc_codeword_symbols[i] = ofdm->rx_np[j];
      }
      ...
      */
      VLA_FREE(symbol_likelihood, ldpc_codeword_symbols);
    }
    ...
    for (i = 0; i < ofdm_nrxbuf; i++) {
      rxbuf_log[ofdm_nrxbuf * f + i] = ofdm->rxbuf[i];
    }

    for (i = 0; i < (ofdm_ns + 3); i++) {
      for (j = 0; j < (ofdm_nc + 2); j++) {
        rx_sym_log[(ofdm_ns + 3) * f + i][j] = ofdm->rx_sym[i][j];
      }
    }
    ...
    for (i = 0; i < ofdm_rowsperframe * ofdm_nc; i++) {
      rx_np_log[ofdm_rowsperframe * ofdm_nc * f + i] = ofdm->rx_np[i];
    }
    ...
    memcpy(&rx_bits_log[ofdm_bitsperframe * f], rx_bits,
           sizeof(int) * ofdm_bitsperframe);
  ...
  VLA_FREE(tx_bits, tx, rx_bits, tx_bits_log, tx_log, rx_log, rxbuf_in_log,
           rxbuf_log, rx_np_log, rx_amp_log, rx_bits_log, symbol_likelihood_log,
           rxbuf_in);
  VLA_FREE_DIM2(rx_sym_log);
