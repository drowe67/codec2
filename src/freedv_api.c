...
int freedv_rawdatarx(struct freedv *f, unsigned char *packed_payload_bits,
  ...
  int nin = freedv_nin(f);
  assert(nin <= f->n_max_modem_samples);
  VLA_CALLOC(COMP, demod_in_comp, f->n_max_modem_samples);
...
struct FSK *freedv_get_fsk(struct freedv *f) { return f->fsk; }
...
#ifndef __EMBEDDED__
    size_t ncopy = (char *)stats->rx_eye - (char *)stats;
