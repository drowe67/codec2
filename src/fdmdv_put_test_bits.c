...
int main(int argc, char *argv[]) {
  ...
  while (fread(packed_bits, sizeof(char), bytes_per_codec_frame, fin) ==
    ...
    fdmdv_put_test_bits(fdmdv, &test_frame_sync, error_pattern, &bit_errors,
                        &ntest_bits, rx_bits);
    ...
    if (test_frame_sync_state) {
      if (test_frame_count == 0) {
        ...
        printf("-");
      test_frame_count++;
      if (test_frame_count == 4) test_frame_count = 0;
    } else
      printf("-");

    fdmdv_put_test_bits(fdmdv, &test_frame_sync, error_pattern, &bit_errors,
                        &ntest_bits, &rx_bits[bits_per_fdmdv_frame]);
  ...
  printf("\nbits %d  errors %d  BER %1.4f\n", total_bits, total_bit_errors,
         (float)total_bit_errors / (1E-6 + total_bits));
